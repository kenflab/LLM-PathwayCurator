#!/usr/bin/env python3
# paper/scripts/run_figSx_collections_pipeline.py
...
def _warn(msg: str) -> None:
    print(f"[run_figSx] WARNING: {msg}", file=sys.stderr)


def _read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        _die(f"[run_figSx] missing file: {path}")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except Exception as e:
        _die(f"[run_figSx] invalid JSON: {path}\n{e}")
...
def _ensure_file(path: Path, label: str) -> None:
    if not path.exists():
        _die(f"[run_figSx] {label} not found: {path}")
    if not path.is_file():
        _die(f"[run_figSx] {label} is not a file: {path}")
    if path.stat().st_size == 0:
        _die(f"[run_figSx] {label} is empty: {path}")
...
def _validate_evidence_table(path: Path) -> None:
    try:
        ...
        _die(f"[run_figSx] invalid EvidenceTable contract: {path}\n{e}")
...
def _make_rotation_map(conds: list[str]) -> dict[str, str]:
    if len(conds) < 2:
        _die("[run_figSx] context_swap requires >=2 conditions (cancers) in this run")
    rot = conds[1:] + conds[:1]
...
def _set_context_review_mode(card: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    if mode not in {"off", "llm"}:
        _die(f"[run_figSx] invalid context_review_mode={mode} (allowed: off,llm)")
...
def _require_backend_env_for_context_review() -> None:
    ...
    if not kind:
        _die(
            "[run_figSx] --context-review-mode llm requires LLMPATH_BACKEND "
            "to be set (openai|ollama|gemini)."
        )
    if kind not in {"openai", "ollama", "gemini"}:
        _die(f"[run_figSx] Unknown LLMPATH_BACKEND={kind!r} (use openai|ollama|gemini)")
...
def _run_one(job: Job, *, force: bool) -> None:
    if (not force) and _report_exists_ok(job.outdir):
        print(f"[run_figSx] SKIP (exists): {job.outdir}")
        return
    ...
    meta = {
        "runner": "paper/scripts/run_figSx_collections_pipeline.py",
...
def main() -> None:
    ...
    if not benchmark_id:
        _die("[run_figSx] --benchmark-id must be non-empty")

    tau = float(args.tau)
    if tau < 0.0 or tau > 1.0:
        _die(f"[run_figSx] --tau must be in [0,1] (got {tau})")

    if int(args.k_claims) < 1:
        _die(f"[run_figSx] --k-claims must be >=1 (got {args.k_claims})")

    cancers = [c.strip().upper() for c in _parse_csv(args.cancers)]
    if not cancers:
        _die("[run_figSx] --cancers must be non-empty")
    ...
    if bad:
        _die(f"[run_figSx] unsupported variants: {bad} (allowed={sorted(allowed_variants)})")
    if not variants:
        _die("[run_figSx] --variants must be non-empty")
    ...
    if badg:
        _die(f"[run_figSx] unsupported gate modes: {badg} (allowed={sorted(allowed_gates)})")
    if not gate_modes:
        _die("[run_figSx] --gate-modes must be non-empty")

    # resolve benchmark dir
    sd = PAPER / "source_data" / benchmark_id
    if not sd.exists():
        _die(f"[run_figSx] benchmark directory not found: {sd}")
    evid_dir = sd / "evidence_tables"
    if not evid_dir.exists():
        _die(f"[run_figSx] missing evidence_tables dir: {evid_dir}")
    ...
    if missing:
        _die(f"[run_figSx] unknown collections: {missing} (known={sorted(all_specs)})")
        ...
        for cancer in cancers:
            ...
            for gm in gate_modes:
                ...
                if "context_swap" in variants:
                    ...
                    if not dst:
                        _die("[run_figSx] internal error: missing swap map entry")
    ...
    n = len(jobs)
    print(f"[run_figSx] jobs: {n}")
    for i, job in enumerate(jobs, start=1):
        print(
            f"[run_figSx] ({i}/{n}) {job.collection} {job.condition} {job.variant} "
