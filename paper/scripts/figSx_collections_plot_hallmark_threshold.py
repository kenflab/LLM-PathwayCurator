#!/usr/bin/env python3
# paper/scripts/figSx_collections_plot_hallmark_threshold.py
...
def _write_summary_tables(res: dict[str, object], *, outdir: Path) -> None:
    ...
    with open(outdir / "FigSx_hallmark_threshold_percentiles.json", "w") as f:
    ...
    pd.DataFrame(rows).to_csv(
        outdir / "FigSx_hallmark_threshold_percentiles.tsv", sep="\t", index=False
    )
